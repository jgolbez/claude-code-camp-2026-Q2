# Week 3: Combat & Leveling — detailed working log

> **This is the full blow-by-blow** (pre-registration, per-slice designs, every
> observation with its prediction, token/flow tables) backing the readable review in
> [3_combat.md](./3_combat.md). Kept for reference; the journal is the review.
...
then grind-spot *rediscovery*, then grind *location* (town is a trap). The acceptance
test (level 4 + train) waits on grind-spot memory routing the whole budget into the
proven loop. Parked: parse_room parens/fingerprint migration, a `move`-level over-level
guard, and the sewer as a second grind ladder.
