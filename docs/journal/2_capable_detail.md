# Week 2: Capable — detailed working log

> **This is the full blow-by-blow** (per-slice predictions, token tables, test
> details) backing the readable journal in [2_capable.md](./2_capable.md). Kept for
> reference; the journal is the review document. Observations record their
> prediction *before building* so each can be checked honestly.
